...
//!
//! Mirrors the estate's canonical sink (keystone/sanctum/relay): [`AuditSink`] holds a bounded
...
//! queue is full or the worker is gone, so a slow/down Watchtower NEVER blocks, slows, or fails the
//! request path or the poller. When audit is disabled (the dev/test default) the sink is a no-op.
//!
//! Pulse stamps only the logical fields (actor=sub / action=`pulse.risk.high` / target / severity /
//! detail). `detail` is short and value-free (score + level) — never raw telemetry.
...
impl AuditEvent {
    ...
    /// `warning`-severity event (a risk verdict that crossed the high threshold).
    pub fn warning(action: &str, actor: &str, target: &str, detail: &str) -> Self {
        Self::new("warning", action, actor, target, detail)
    }
}

/// Non-blocking audit sink. Cheap to clone. A disabled sink (`inner == None`) makes [`emit`] a
/// no-op.
#[derive(Clone)]
...
impl AuditSink {
    ...
    /// Build the sink from config and (when enabled) spawn the background worker. Returns a disabled
    /// sink when `enabled` is false, when no ingest `token` is set, or when `watchtower_url` is
    /// unparseable — those misconfigurations only warn and turn audit OFF; they never fail startup
    /// or affect the request path. Must be called from within a tokio runtime when enabled.
    ...
    /// worker the event is DROPPED (drop counter + warn). NEVER blocks or errors the caller.
    ...
    /// Total events dropped so far. `0` for a disabled sink.
...
/// Drain the queue and POST each event. Delivery failures only warn — never retried, never affects
/// anything but this background task.
...
/// Parsed Watchtower target: where to connect, the `Host` authority, and the request `path`.
struct Target {
    host: String,
    port: u16,
    authority: String,
...
mod tests {
    ...
    fn event_serializes_to_safe_fields() {
        ...
        assert_eq!(v["action"], "pulse.risk.high");
        assert_eq!(v["source"], "pulse");
    }
